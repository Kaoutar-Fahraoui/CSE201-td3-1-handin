...
double* extend_array(double* array, int length, int new_size) {
    double* big_array= new double[new_size];
...
double* shrink_array(double* array, int length, int new_size) {
    double* small_array= new double[new_size];
    for(int i=0; i<new_size;i++){
...
double* append_to_array(double element,
    ...
    if(current_size==max_size){
        ...
        new_array=extend_array(array,max_size, max_size+5);
        max_size+=5;
...
double* remove_from_array(double* array,
    ...
    if(abs(current_size-max_size)>=5){
        new_array=shrink_array(array,max_size, max_size-5);
        max_size-=5;
    }
    else{
        new_array=array;
    ...
    for (int i=0; i<global_telemetry_current_size;i+=3){
        for (int j=0; j<global_telemetry_current_size; j+=3){
            if (global_telemetry[i]<global_telemetry[j]){
                double rap_i=global_telemetry[i];
                double rap_i1=global_telemetry[i+1];
                double rap_i2=global_telemetry[i+2];

                global_telemetry[i]=global_telemetry[j];
                global_telemetry[i+1]=global_telemetry[j+1];
                global_telemetry[i+2]=global_telemetry[j+2];

                global_telemetry[j]=rap_i;
                global_telemetry[j+1]=rap_i1;
                global_telemetry[j+2]=rap_i2;
